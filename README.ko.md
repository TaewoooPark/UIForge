...
<p align="center">
  <strong>웹사이트를 <em>실제로 작동하게</em> 복제한다 — 탭·필터·목록·클릭 전환·모션·스크롤까지. 그리고 픽셀 충실 <em>동결본</em>과 픽셀 동일·편집 가능한 React <em>복원본</em>도.</strong><br>
...
| | 무엇인가 | 용도 |
|---|---|---|
| **⭐ 아카이브** (`uiforge-archive`) | **완전한 동작** — 사이트 자신의 코드 + 녹화한 모든 응답을 **열람·수정 가능한 미러**(진짜 파일·진짜 파일명, 불투명 blob이 아님)에서 오프라인 재생 | **작동하는** 복제: 탭·필터·목록·클릭 전환·모션·스크롤 |
| **동결본** (`uiforge-freeze`) | 자립형·**픽셀 충실** 정지본. 진짜 CSS·폰트·에셋 보존, 스크립트 제거, 시간 정지 | 정확한 오프라인 정지 화면, 그리고 재건본을 재는 **오라클** |
| **복원본** (`uiforge-restore` · `-sourcemap`) | **픽셀 동일**하게 렌더되는 편집 가능한 소스 — 진짜 DOM을 클래스 **byte 단위 그대로** 유지한 채 React로 컴포넌트화하거나, 소스맵을 흘린 사이트면 **진짜 원본 소스** 복원 | 편집·개발·당신 콘텐츠로 배포 — 충실도 드리프트 없이 |

**아카이브는 진짜 JavaScript를 캐시된 데이터에 대고 실행**한다. 그래서 모든 인터랙션이 원본 그대로 동작한다 — 재구성이 아니라 *그게 곧 그 동작*이기 때문. (이게 "Save As"를 이기는 이유다. 모던 앱을 순진하게 저장하면 앱이 fetch하는 XHR·API 데이터가 안 담겨 깨진 껍데기로 열린다. 아카이브는 그 데이터를 녹화하고 *재생*한다.) 동결본은 라이브와 동일하게 렌더되고, **복원본은 진짜 DOM + 진짜 CSS를 유지해 설계상(by construction) 픽셀 동일**하다 — 실측: ui.shadcn.com은 **0.00% 픽셀 차이**로 복원되고, 소스맵을 흘린 사이트면 **진짜 원본 `.tsx`**까지 복원한다.

---

## 편집 가능한 코드로 복원 — 게다가 *픽셀 동일*

**복원본**을 아카이브에 겨누면, **원본과 똑같이 렌더되는 편집 가능한 React 프로젝트**가 나온다. 진짜 DOM, 진짜 클래스 문자열(byte 단위), 진짜 컴파일된 CSS를 그대로 유지하기 때문 — 충실도는 LLM으로 쫓는 목표가 아니라 *기본값*이다. 모든 결과는 오프라인 픽셀 게이트로 **증명**한다:

| 사이트 | 복원 형태 | 원본 대비 픽셀 차이 |
|---|---|---|
| **ui.shadcn.com** | React, 클래스 그대로 | **0.00% — 픽셀 동일** |
| **vercel.com** | ″ | 1.8% *(WebGL canvas 히어로만 — JS 필요)* |
| **linear.app** | ″ | 5% *(JS 상태 메뉴 — Tier A가 진짜 소스로 복원 ↓)* |

그리고 사이트가 **소스맵을 흘리면**, 최상위 티어가 **진짜 원본 소스**를 복원한다 — 원래 이름·주석·타입이 살아있는 진짜 `.tsx`. linear.app → **421개 진짜 소스 파일**, 정적 렌더로는 재현 못하는 그 `MobileMenu.tsx`(`useState` 열림 로직 포함)까지. 두 티어, 좋은 것부터, 전부 아카이브에서 — 오프라인으로:
...
| **편집 가능**·**픽셀 동일** 소스 | `--restore` (또는 `--react`), 또는 *"…내 제품용으로 (product.md)"* | 진짜 DOM → React(클래스 byte 그대로) + 소스맵으로 진짜 원본 소스 — 픽셀 게이트로 증명 |
